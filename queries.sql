...
on s.product_id = p.product_id
group by seller -- группируем данные по продавцам
having avg(s.quantity * p.price) < (select avg(sl.quantity * pr.price) as mid -- считаем средний доход по всем
                                            from products as pr 
...
select 
	...
	count(*) as age_count -- считаем количество покупателей
...
select to_char(s.sale_date, 'YYYY-MM') as selling_month, -- выбираем год и месяц из столбца с датамиcount(s.product_id) as total_products, -- считаем количество уникальные покупателей
	floor(sum(s.quantity * p.price)) as income -- считаем доход и округляем его в меньшую сторону
from sales as s -- выбираем таблицу с продажами
inner join products as p -- присоединяем таблицу с продуктами
on s.product_id  = p.product_id 
